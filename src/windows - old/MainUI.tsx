...
/* ---------------- Default_Preset ---------------- */
const PREFS_STORAGE_KEY = "splitwriter:preferences";
...
function loadPrefsFromStorage(defaults: PrefsType): PrefsType {
  try {
    ...
    if (raw) {
      const parsed = JSON.parse(raw) as Partial<PrefsType>;
      return {
        ...defaults,
        workingFolder: wfKey ?? parsed.workingFolder ?? defaults.workingFolder,
        autosave: parsed.autosave ?? defaults.autosave,
        autosaveIntervalSec: parsed.autosaveIntervalSec ?? defaults.autosaveIntervalSec,
        typeface: {
          headline: parsed.typeface?.headline ?? defaults.typeface.headline,
          body:     parsed.typeface?.body     ?? defaults.typeface.body,
          accent:   parsed.typeface?.accent   ?? defaults.typeface.accent,
          etc:      parsed.typeface?.etc      ?? defaults.typeface.etc,
        },
        accentColor: defaults.accentColor,
        writingGoal: parsed.writingGoal ?? defaults.writingGoal,
        bracket:     parsed.bracket     ?? defaults.bracket,
...
export default function MainUI() {
  ...
  const MSG_OPEN = "Open a project? Unsaved changes will be lost.";
  const MSG_NEW  = "Create a new project? Unsaved changes will be lost.";
  const MSG_QUIT = "There are unsaved changes. Quit without saving?";
...
const applySnap = (s: HistorySnap) => {
  ...
  async function onExportPrint(provider?: () => string) {
    ...
    const onMsg = (ev: MessageEvent) => {
      if (ev?.data?.who !== "splitwriter") return;

      if (ev.data.type === "SW_PRINT_OPENING") {
        try { hideWait(); } catch {}
        window.removeEventListener("message", onMsg);
        io?.notify?.('Opened print dialog — choose “Save as PDF”.', "info", 2200);
      }
      if (ev.data.type === "SW_PRINT_CLOSED") {
        try { hideWait(); } catch {}
        window.removeEventListener("message", onMsg);
      }
    };
    ...
    const presetScale = tf ? {
      1: Math.max(0.5, tf.headline.size / bodyPx),
      2: 1,
      3: Math.max(0.5, tf.accent.size   / bodyPx),
      4: Math.max(0.5, tf.etc.size      / bodyPx),
    } : undefined;
  ...
  const hasWorkContent = React.useCallback((): boolean => {
    const editors = document.querySelectorAll<HTMLElement>(
      '[data-board-id] [contenteditable="true"]'
    );
    ...
    for (const v of Object.values(imageDocsRef.current)) {
  ...
  const notifyImpl = React.useCallback(
    (text: string, level: "info" | "warn" | "error" = "info", ttl = 0) => {
      ...
      window.dispatchEvent(new CustomEvent("sw:status", { detail: { text: msg, level, ttl } }));
    },
    []
  );
  ...
  const saveAsAndBind = React.useCallback(async () => {
    await io.saveAs();
    if ((window as any).showSaveFilePicker) hasBoundFileRef.current = true;
  }, [io]);

  const quickSave = React.useCallback(async () => {
    await io.save();
  }, [io]);
  ...
  React.useEffect(() => {
    ...
    const onKey = async (e: KeyboardEvent) => {
      ...
      if ((ctrl && k === "r") || k === "f5") {
        ...
        const unsaved = io.isDirty();
        if (unsaved) {
          const ok = await ask("Save before reload?");
          if (!ok) return;  
          try { await io.save(); } catch (err) { console.error(err); return; } 
        }
      ...
      // ───────── File ops ─────────
      if (ctrl && k === "s" && !e.shiftKey && !e.altKey) { e.preventDefault(); quickSave();     return; }
      if (ctrl && k === "s" &&  e.shiftKey && !e.altKey) { e.preventDefault(); saveAsAndBind(); return; }
      if (ctrl && k === "o" && !e.shiftKey && !e.altKey) {
        e.preventDefault();
        const unsaved = io.isDirty();
        if (unsaved && !(await ask(MSG_OPEN))) return;
        io.open();
        return;
      }
      if (ctrl && k === "n" && !e.shiftKey && !e.altKey) {
        e.preventDefault();
        const unsaved = io.isDirty();
        if (unsaved && !(await ask(MSG_NEW))) return;
        io.newFile();
      ...
      if (ctrl && (k === "z" || k === "y" || (k === "z" && e.shiftKey))) {
        const imageId = lastImageFocusRef.current;
        if (imageId) {
          e.preventDefault();
          // undo / redo
          const next = (k === "z" && !e.shiftKey) ? imageUndo(imageId) : imageRedo(imageId);
          if (next !== undefined) { 
            setImageDocs(m => {
      ...
      if (ctrl && k === "z") { if (inEditor) return; e.preventDefault(); undo(); return; }
      if (ctrl && k === "y") { if (inEditor) return; e.preventDefault(); redo(); return; }
      if (ctrl && k === "\\") { e.preventDefault(); setShowSidebar(v => !v); return; }
    };
      ...
      try {
        if ((window as any).__TAURI_IPC__) {
          ...
          unlisten = await appWindow.onCloseRequested(async (event) => {
            ...
            const ok = await ask("Save before exit?");
            if (!ok) {
              const discard = await ask("Quit without saving?");
              if (discard) appWindow.close();
              return;
            }
            try { await io.save(); } catch (err) {
              console.error(err);
              return;    
            }
            appWindow.close();  
  ...
  React.useEffect(() => {
    ...
    (async () => {
      unsubs.push(await listen("sw:new", async () => { /* ... */ }));
      unsubs.push(await listen("sw:open", async () => {
        await io.open();
        if ((window as any).showOpenFilePicker) hasBoundFileRef.current = true;
      }));
      unsubs.push(await listen("sw:save",   () => quickSave()));
      unsubs.push(await listen("sw:saveas", () => saveAsAndBind()));
      unsubs.push(await listen("prefs-changed", () => {
        try {
          const next = loadPrefsFromStorage(DEFAULT_PREFS);
          setPrefs(next);
          if (next?.workingFolder) setWorkingFolder(next.workingFolder);
        } catch {}
      }));
      unsubs.push(await listen("sw:quit", async () => {
        const unsaved = io.isDirty();
        if (unsaved && !(await ask(MSG_QUIT))) return;
        try {
          if ((window as any).__TAURI_IPC__) {
            const { appWindow } = await import("@tauri-apps/api/window");
        ...
        if (hasBoundFileRef.current) {
          try {
            if (io.isDirty()) {
              ...
              await io.save();
              io.notify("Auto-saved.", "info", 1200);
            }
          } catch (e) {
            console.error(e);
          } finally {
  ...
  // split ratio
  const onChangeRatio = (ratio: number, path: number[]) => {
    setTree((t) => updateRatioByPath(t, path, ratio));
    io.markDirty();
  };

  const handleRequestSplit = (leafId: string, dir: "horizontal" | "vertical") => {
  ...
  function openBoardHere(targetLeafId: string, srcTextId: string, html: string) {
    ...
    // Create buffer if target’s previous board buffer is missin
  ...
  const GUTTER_PX = 2;     // Same as SplitPane.gutterSize
  const LEAF_MIN_PX = 48;  // Minimum leaf size (roughly handle size)

  function subtreeMinWidth(n: TreeNode): number {
    if (n.type === "leaf") return LEAF_MIN_PX;
    if (n.dir === "vertical") {
      // 좌우로 나뉘면, 각각의 최소폭이 그대로 합쳐져야 함(+가터)
      return subtreeMinWidth(n.a) + GUTTER_PX + subtreeMinWidth(n.b);
    } else {
      // 위아래로 나뉘면, 전체 너비 최소는 자식 중 큰 쪽
  ...
  const renderNode = (node: TreeNode, path: number[] = []): React.ReactNode =>
    node.type === "leaf" ? (
      ...
      <SplitPane
        ...
        direction={node.dir}
        ratio={node.ratio}
        onChange={(r) => onChangeRatio(r, path)}
        gutterSize={GUTTER_PX}
        minA={subtreeMinWidth(node.a)}   // Min width (px) of subtree A
        minB={subtreeMinWidth(node.b)}   // Min width (px) of subtree B
        a={renderNode(node.a, [...path, 0]) as any}
        b={renderNode(node.b, [...path, 1]) as any}
      />
    );
  ...
  const menuItems = React.useMemo(() => {
    ...
    if (menu.pane === "root") {
      ...
      if (leaf.kind === "image") {
        return [
          { label: "Change Board Type >", onClick: keepOpenOnce(() => gotoPane("image:changeType")) },
          {
            label: "Duplicate to subfolder",
            onClick: () => {
              if (!workingFolder) {
                io.notify("Save this project as a .swon first to duplicate images.");
                return;
              }
    ...
    if (menu.pane === "text:export") {
      ...
      return [
        ...
        { label: "PDF", onClick: () => onExportPrint(/* provider */) },
      ];
    }
  ...
  return (
    <div data-app-root
      ...
      onMouseDown={() => {
        window.__CTX_TARGET__ = null;        // [ADD]
    ...
    >
      ...
      >
              ...
              <Sidebar
                ...
                onOpenFile={async (absPath: string) => {
                  try {
                    ...
                    try { (window as any).__SW_CURRENT_FILE__ = absPath; } catch {}
                    try { window.dispatchEvent(new CustomEvent("sw:file:opened", { detail: { path: absPath } })); } catch {}
        ...
        {showAbout && (
          <Modal onClose={() => setShowAbout(false)} width={420}>
            <AboutPanel />
          </Modal>
        )}
      </div>
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{ marginBottom: 10 }}>
        ...
        <div style={{ fontSize: 12, opacity: 0.80,marginTop: 2, textAlign: "left" }}>
          For all creators — past, present, and future.
        </div>

        <div style={{ fontSize: 12, opacity: 0.65, marginTop: 4, textAlign: "left" }}>
          v0.9.0
        </div>

        <div style={{ fontSize: 11, opacity: 0.60, marginTop: 2, textAlign: "right" }}>
          Created by Crom & GPT
        </div>
      ...
      <div
        style={{ height: 1, background: "rgba(255,255,255,0.08)", margin: "10px 0 12px" }}
      />
      ...
      <Section title="PRESET">
        <Row k="Headline (H)" v={<><Keys parts={["Ctrl", "1"]} />&nbsp;or&nbsp;<Keys parts={["Alt", "1"]} /></>} />
        <Row k="Body (B)"     v={<><Keys parts={["Ctrl", "2"]} />&nbsp;or&nbsp;<Keys parts={["Alt", "2"]} /></>} />
        <Row k="Accent (A)"   v={<><Keys parts={["Ctrl", "3"]} />&nbsp;or&nbsp;<Keys parts={["Alt", "3"]} /></>} />
        <Row k="Etc (E)"      v={<><Keys parts={["Ctrl", "4"]} />&nbsp;or&nbsp;<Keys parts={["Alt", "4"]} /></>} />
        <div style={{ fontSize: 10, opacity: 0.65, marginTop: 6 }}>
          If text is selected, only the selection changes. If nothing is selected, the style applies to text you type next.
        </div>
      </Section>
      ...
      <Section title="CONTEXT MENU (TEXT BOARD)">
        <Row
          k="Copy / Paste / Typewriter / Spell Checker"
          v="Right Click"
        />
        <div style={{ fontSize: 10, opacity: 0.65, marginTop: 6 }}>
      ...
      <Section title="CONTEXT MENU (IMAGE BOARD)">
        <Row
          k="Change Image / Reset position / Use as Echo background"
          v="Right Click"
        />
        <div style={{ fontSize: 10, opacity: 0.65, marginTop: 6 }}>
