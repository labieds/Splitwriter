...
export function setupSwonIO(opts: Opts): SwonIO {
  ...
  /** Snapshot current app state into a SWON object. */
  function buildSwon(): SwonFile {
    ...
    for (const [id, im] of Object.entries(imgsIn)) {
      ...
      // Normalize what to persist in `src`:
      // - blob:/data: → prefer absolute file path if known
      // - ephemeral app URLs → keep absolute file if known; otherwise keep URL
      // - absolute filesystem path → keep as-is
      // - otherwise → src or fallback to file path
      ...
      // `file` keeps an absolute path when available (helps future reloads).
