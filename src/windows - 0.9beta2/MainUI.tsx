...
const AppVersion: React.FC<{ fallback?: string }> = ({ fallback = "v0.9.1-beta.2" }) => {
...
const applySnap = (s: HistorySnap) => {
  ...
  async function getProjectDirNow(): Promise<string | null> {
    if (!(window as any).__TAURI_IPC__) return workingFolder || null;
    ...
    try { return workingFolder || localStorage.getItem(PREFS_WF_KEY) || null; } catch { return workingFolder || null; }
  }
              ...
              <Sidebar
                ...
                onOpenFile={async (absPath: string) => {
                  try {
                    ...
                    // 여기서는 “지금 이 프로젝트를 열었다”는 사실만 UI에 반영
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{ marginBottom: 10 }}>
        ...
        <AppVersion fallback="v0.9.1-beta.2" />

        <div style={{ fontSize: 9, opacity: 0.60, marginTop: 2, textAlign: "right" }}>
          Created by Crom & GPT
        </div>
