...
const AppVersion: React.FC<{ fallback?: string }> = ({ fallback = "v0.9.0-beta.1" }) => {
...
const applySnap = (s: HistorySnap) => {
 ...
 async function duplicateImageToProject(imageId: string) {
   if (!workingFolder) { io.notify("Save this project as a .swon first to duplicate images."); return; }
   ...
   const outDir  = await join(workingFolder, "_image");
  ...
  React.useEffect(() => {
    ...
    const onKey = async (e: KeyboardEvent) => {
      ...
      if ((ctrl && k === "r") || k === "f5") {
        e.preventDefault();
          await reloadWithGuard();
          return;
        }

      // ───────── File ops ─────────
      if (ctrl && k === "s" && !e.shiftKey && !e.altKey) { e.preventDefault(); await save();            return; }
      if (ctrl && k === "s" &&  e.shiftKey && !e.altKey) { e.preventDefault(); await saveAsAndBind();   return; }
      if (ctrl && k === "o" && !e.shiftKey && !e.altKey) { e.preventDefault(); await openWithGuard();   return; }
      if (ctrl && k === "n" && !e.shiftKey && !e.altKey) { e.preventDefault(); await newWithGuard();    return; }
  ...
  React.useEffect(() => {
    ...
    (async () => {
      unsubs.push(await listen("sw:new",     async () => { await newWithGuard(); }));
      unsubs.push(await listen("sw:open",    async () => { await openWithGuard(); }));
      unsubs.push(await listen("sw:save",    async () => { await save(); }));
      unsubs.push(await listen("sw:saveas",  async () => { await saveAsAndBind(); }));
      ...
      unsubs.push(await listen("sw:quit",    async () => { await quitWithGuard(); }));
    })();
          ...
          <div style={{ padding: 16 }}>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 14 }}>
              <button
                ...
                style={{ padding: "6px 10px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.16)", background: "rgba(255,255,255,0.08)", color: "#e6e6e6" }}
              >
                Save and Quit
              </button>
              <button
                onClick={() => { ask3State.resolve?.("discard"); setAsk3State({open:false, msg:""}); }}
                style={{ padding: "6px 10px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.16)", background: "rgba(255,255,255,0.06)", color: "#e6e6e6" }}
              >
                Quit without Save
              </button>
              <button
                onClick={() => { ask3State.resolve?.("cancel"); setAsk3State({open:false, msg:""}); }}
                style={{ padding: "6px 10px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.16)", background: "transparent", color: "#e6e6e6" }}
              >
                Cancel
              </button>
            </div>
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{ marginBottom: 10 }}>
        ...
        <AppVersion fallback="v0.9.0-beta.1" />
