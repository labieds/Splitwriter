...
async function appOpen(absPath: string) {
  try {
    ...
    const fn =
      mod.openWithGuard ||
      mod.openFileWithGuard ||
      mod.openSwonWithGuard ||
      mod.openByPath ||
      mod.openFile ||
      mod.openSwon ||
      mod.open;
    if (typeof fn === "function") return await fn(absPath);
  } catch {}
...
export default function Sidebar({
  ...
  const openAndRemember = async (abs: string) => {
    ...
    // 1) 부모 콜백이 있으면(기존 흐름) 우선 사용
    if (onOpenFile) { onOpenFile(abs); return; }

    // 2) appActions 가 있으면 그 쪽으로 열기(더티가드 포함 이름 우선)
    await appOpen(abs);
  };
