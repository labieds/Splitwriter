...
function tokenizeEchoText(src?: string): string[] {
  ...
  for (const p of paras) {
    ...
    while ((m = re.exec(p)) !== null) { out.push(m[1].trim()); last = re.lastIndex; }
    const rest = p.slice(last).trim(); if (rest) out.push(rest);
  }
  return out;
}
...
const tfByPreset = (
  ...
  const key = preset === 1 ? "headline" : preset === 3 ? "accent" : preset === 4 ? "etc" : "body";
...
const EchoReader: React.FC<EchoReaderProps> = ({
  ...
  useEffect(() => { if (open) setIdx(0); }, [open, start]);
  ...
  useEffect(() => { if (open) setSizeDelta(Number(sizeOffset || 0)); }, [open, sizeOffset]);
  ...
  // use an integer pixel for visual crispness
  const ringPx = Math.max(2, Math.round(borderWidth));
  const boxFill = `color-mix(in oklab, ${boxBg} ${Math.round(safeBoxOpacity * 100)}%, transparent ${Math.round((1 - safeBoxOpacity) * 100)}%)`;

  // Current background image URL (if any).
  const bgUrl = (bgSources && bgSources.length > 0)
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      // Background cycle
      else if (e.key === "]" && (bgSources?.length ?? 0) > 0) { e.preventDefault(); setBgIndex(i => ((i + 1) % (bgSources!.length))); }
      else if (e.key === "[" && (bgSources?.length ?? 0) > 0) { e.preventDefault(); setBgIndex(i => (i - 1 + bgSources!.length) % bgSources!.length); }
      // Font size (session only)
      else if (e.key === "+" || e.key === "=") { e.preventDefault(); setSizeDelta(v => Math.min(v + 1, 18)); }
      else if (e.key === "-" || e.key === "_") { e.preventDefault(); setSizeDelta(v => Math.max(v - 1, -12)); }
    };
    window.addEventListener("keydown", onKey, { capture: true });
    return () => window.removeEventListener("keydown", onKey as any, { capture: true } as any);
  }, [open, onClose, next, prev, bgSources?.length]);

  if (!open || view.length === 0 || !hostRef.current) return null;
  ...
  return createPortal(
    ...
    >
      ...
      {/* Center text box */}
      <div style={{ display:"flex", alignItems:"center", justifyContent:"center", padding:"0 24px", zIndex: 1 }}>
        <div
          style={{
            width: "100%",
            ...
            background: boxFill,
            borderRadius: radius,
            // shadow ring acts like a smoother border
            border: "none",
            boxShadow: `0 0 0 ${ringPx}px ${boxBorder}, 0 10px 30px rgba(0,0,0,.55)`,
            // give the text some top air
